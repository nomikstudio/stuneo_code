...
// Station abspielen und in localStorage speichern
function playStation(stationId, stationName, streamUrl, logoUrl = '', ownerName = 'Unknown', ownerSlug = '#', saveToLocalStorage = true) {
    ...
    // Prüfen, ob dieselbe Station bereits läuft und dann umschalten
    if (currentStationId === stationId) {
        togglePlayPause();
        return;
    }

    // Stoppe die aktuelle Wiedergabe und setze Parameter zurück
    if (currentAudio) {
        currentAudio.pause();
    ...
    // Setze die ID der aktuellen Station und das Audio-Element
    ...
    console.log("Calling updateOwnerInfo with:", ownerName, ownerSlug);
    updateOwnerInfo(ownerName, ownerSlug);

    // Setze Lautstärke
    const volumeSlider = document.getElementById('volumeSlider');
    if (volumeSlider) currentAudio.volume = volumeSlider.value || 1;
    ...
    // Debugging: Überprüfen, ob die Station in localStorage korrekt gespeichert wird
    if (saveToLocalStorage) {
        ...
        console.log("Saving to localStorage:", stationData);
...
function updateOwnerInfo(ownerName, ownerSlug) {
    ...
    // Setze den Link und Tooltip, wenn slug vorhanden ist, ansonsten auf '#' setzen
    const ownerUrl = ownerSlug ? `radio-owner?slug=${ownerSlug}` : '#';

    if (ownerLink) ownerLink.href = ownerUrl;
    if (ownerTooltip) ownerTooltip.innerText = ownerName || "Unknown";
    if (ownerNameElement) ownerNameElement.innerText = ownerName;
    if (overlayOwnerLink) overlayOwnerLink.href = ownerUrl;
    if (overlayOwnerNameElement) overlayOwnerNameElement.innerText = ownerName || "Unknown";
}
